...
d.text((left_cx, y), "Programme of Visitations",
       font=sc(70), fill=GOLD_DARK, anchor="ma")
y += 96

intro = "Condolences will be received from 4:00 PM to 8:00 PM :"
for line in wrap(intro, f(32), 1000):
    d.text((left_cx, y), line, font=f(32), fill=INK_SOFT, anchor="ma")
    y += 46
y += 12

d.text((left_cx, y), "Every Thursday, Friday and Saturday",
       font=f(34), fill=INK_SOFT, anchor="ma")
y += 48
d.text((left_cx, y), "from 28 May to 20 June 2026",
       font=f(34), fill=INK_SOFT, anchor="ma")
y += 60
d.text((left_cx, y), "From Monday 22 to Wednesday 24 June 2026",
       font=f(34), fill=INK_SOFT, anchor="ma")
y += 64

tracked(d, left_cx, y, "VENUE", f(26), SUBTLE, track=12)
y += 42
d.text((left_cx, y), "The TOGBEY Family Residence",
       font=fb(36), fill=INK, anchor="ma")
y += 50
d.text((left_cx, y), "Tokoin Kodomé District · 3rd lane on the right · opposite the market",
...
# Obsèques header
d.text((left_cx, y), "Funeral Programme",
       font=sc(70), fill=GOLD_DARK, anchor="ma")
y += 96
...
events_by_day = [
    ("Friday 26 June 2026", [
        ("6:30 PM",
         "Vigil of prayers and hymns at the **Baptist Bible Church of "
         "Tokoin-Solidarité**, followed by the viewing of the body at the "
         "**family residence**."),
    ]),
    ("Saturday 27 June 2026", [
        ("7:00 AM",
         "Procession of the body, followed by the funeral and "
         "thanksgiving mass at the same church."),
        ("1:00 PM",
         "Burial at **Agomé-Glozou** (Bas-Mono)."),
    ]),
]
...
for di, (date, events) in enumerate(events_by_day):
    if di > 0:
        y += 28
    d.text((left_cx, y), date, font=fs(42 if 1080 < 1200 else 46), fill=GOLD_DARK, anchor="ma")
    y += 64 if 1080 < 1200 else 68
...
d.text((right_cx, y), "In Loving Memory of Our Dearly Beloved",
...
dates_text = "Called home to God on 24 May 2026 at CHU Sylvanus Olympio, in his 78th year."
df = fi(38)
...
# tribute — 64 → 68
d.text((right_cx, y), "You will forever remain in our hearts.",
       font=sc(68), fill=INK_SOFT, anchor="ma")
# Rest in Peace pinned to CLOSING_Y to mirror We love you!
d.text((right_cx, CLOSING_Y), "Rest in Peace.", font=sc(82), fill=GOLD_DARK, anchor="ma")

img.save("flyer_spread_en.png", dpi=(300, 300))
